---
title: 'Node.js SDK'
description: 'Official Node.js SDK for Scapester API'
---

## Overview

Scapester Node.js SDK provides a simple wrapper around the Scapester API to help you transform web content into structured data.

## Installation

Install the Scapester Node.js SDK using npm:
...
1. Get an API key from [scapester.com](https://scapester.com)
2. Set the API key as an environment variable `SCAPESTER_API_KEY` or pass it directly to the ScapesterApp class

Here's a complete example with error handling:

<CodeGroup>
  ```typescript TypeScript
  import { ScapesterApp, ScrapeParams, ScrapeResponse } from 'scapester';

  const app = new ScapesterApp({apiKey: "sk-YOUR_API_KEY"});
  ...
  const { ScapesterApp } = require('scapester');
